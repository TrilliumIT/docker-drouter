...
func (st *simulation) runV4() error {
	...
	defer st.require.NoError(cleanup(), "Failed to run cleanup().")
	defer resetGlobals()
	...
	for i := 0; i < 3; i++ {
		...
		defer st.require.NoError(dc.NetworkRemove(bg, st.n[i].ID), "Failed to remove n%v.", i)
	}
	...
	for i := 0; i < 2; i++ {
		...
		defer st.assert.NoError(st.c[i].remove(), "Failed to remove c%v.", i)
	...
	defer st.assert.NoError(dc.NetworkRemove(bg, st.n[3].ID), "Failed to remove n3.")
